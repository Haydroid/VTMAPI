...
public class DES {
    ...
    public static int[] string2Binary(String source) {
        ...
        DesArray desArray = new DesArray(len);

        DesArrayStr desArrayStr = new DesArrayStr();
        char[] arr = source.toCharArray();

        for (int i = 0; i < len; i++) {

            int t = 0;
            DesArrayStr desArrayInt = new DesArrayStr();
            try {
            ...
            //desArrayStr.setStr(Integer.toBinaryString(desArrayInt.getT()).split(""));
            desArrayStr.setStr(keyPattern.split(Integer.toBinaryString(desArrayInt.getT()), 0));

            int k = i * 4 + 3;

            for (int j = desArrayStr.getStr().length - 1; j > 0; j--) {

                //dest[k] = Integer.parseInt(str[j]);
                desArray.setValue(k, Integer.parseInt(desArrayStr.getStr()[j]));
                k--;
     ...
     * @param source
     * @param key
     * @param mode   0:encrypt加密 1:discrypt解密
     * @return
     */
    public static String des1(String source, String key, int mode) {
//      System.out.println("des解密:"+source+"  "+key);
        ...
        throw new IllegalArgumentException("非法的操作模式");
     ...
     * @return
     */
    public static String des2(String source, String key, int mode) {

        throw new UnsupportedOperationException();
    }

    /**
     * 三重DES算法(双倍长密钥16字节) 密钥K1和K2
     * 1、先用K1加密明文
     * 2、接着用K2对上面的结果进行解密
     * 3、然后用K1对上面的结果进行加密
     *
     * @param source
     * @param key
     * @param mode   0:encrypt加密  1:decrypt解密
     * @return
     */
    public static String des3(String source, String key, int mode) {
        ...
        if (mode == 0) {
            ...
            temp = encryption(source, K1);//加密

            temp = discryption(temp, K2);//解密
    ...
    /**
     * ANSI_X99_MAC(计算MAC(hex) ANSI-X9.9-MAC(16的整数不补) PBOC-DES-MAC(16的整数补8000000000000000))
     * 使用单倍长密钥DES算法
     *
     * @param key  密钥 (16byte)
     * @param data 数据
     * @param type 1-ASC 0-HEX
     * @return String  mac
     * @throws Exception
     * @throws
     * @since 1.0.0
     */
    public static String ANSI_X99_HEX_MAC(String key, String data,
                                          int type)
            throws Exception {
        if (key.length() != 16) {
            return null;
    ...
    public static String getDPK4Once(String data, String mpk) {
        ...
        // 获取DPK右半边
        dpk.append(divData(reverse(data), mpk, 0));
        return dpk.toString();
    }
     ...
     * getDPK(获取TAC子密钥)
     *
     * @param nodeNo 分散因子 机构编号
     * @param cmpNo  分散因子 分公司编号
     * @param uid    分散因子 UID
     * @param mpk    根密钥
     * @return String
     * @throws
     * @since 1.0.0
     */
    public static String getDPK(String nodeNo, String cmpNo, String uid,
                                String mpk) {
        // 第一次分散
        StringBuffer issuerMPK = new StringBuffer();
        // 获取Issuer MPK左半边
        issuerMPK.append(divData(cmpNo, mpk, 0));
        // 获取Issuer MPK右半边
        issuerMPK.append(divData(reverse(cmpNo), mpk, 0));
        // 第二次分散
        StringBuffer cmpMPK = new StringBuffer();
        // 获取DPK左半边
        cmpMPK.append(divData(nodeNo, issuerMPK.toString(), 0));
        // 获取DPK右半边
        cmpMPK.append(divData(reverse(nodeNo), issuerMPK.toString(), 0));

        // 第三次分散
        StringBuffer dpk = new StringBuffer();
        // 获取DPK左半边
        dpk.append(divData(uid, cmpMPK.toString(), 0));
        // 获取DPK右半边
        dpk.append(divData(reverse(uid), cmpMPK.toString(), 0));
        return dpk.toString();
    }

    /**
     * zhouhui add 2013-7-17
     * 获取外部认证子密钥
     *
     * @param cardSerialNo 卡片序列号
     * @param mpk          外部认证主密钥
     * @return 外部认证子密钥
     */
    public static String getExtendKey(String cardSerialNo, String mpk) {
        String dispersionFactor = "00000000" + cardSerialNo;
        StringBuffer extendKey = new StringBuffer();
        // 获取DPK左半边
        extendKey.append(divData(dispersionFactor, mpk, 0));
        // 获取DPK右半边
        extendKey.append(divData(reverse(dispersionFactor), mpk, 0));
        return extendKey.toString();
    }

    /**
     * zhouhui add 2013-7-23
     * 获取MAC1、 MAC2
     *
     * @param data  计算数据
     * @param seSlk 圈存子密钥
     ...
     *              flag = 2 计算Mac2
     */
    public static String getCountMac(String data, String seSlk, String flag) {//
        String initData = "0000000000000000";
        if ("1".equals(flag)) {//计算Mac1
            String d1 = data.substring(0, 16);
            String d2 = data.substring(16, 32);
            String temp1 = des1(xOr(initData, d1), seSlk, 0);//加密
            String temp2 = des1(xOr(temp1, d2), seSlk, 0);//加密
            return temp2.substring(0, 8);
        } else {
            String d1 = data.substring(0, 16);
            String d2 = data.substring(16, 32);
            String d3 = data.substring(32, 48);
            String temp1 = des1(xOr(initData, d1), seSlk, 0);//加密
            String temp2 = des1(xOr(temp1, d2), seSlk, 0);//加密
            String temp3 = des1(xOr(temp2, d3), seSlk, 0);//加密
            return temp3.substring(0, 8);
        }
    }
     ...
     * 3DES 计算mac
     *
     * @param key    32位  主控密钥或维护密钥
     * @param vector 初始向量 随机数+00000000
     * @param data   数据
     * @param type   0 代表16进制
     * @return
     */
    public static String PBOC_3DES_MAC(String key, String vector, String data,
        ...
        I = des3(I, key, 0);
        String strmac = I.substring(0, 8);
        return strmac;
    }

    public static void main(String args[]) throws Exception {
        System.out.println(DES.des3("0379845530160D95", "11111111111111111111111111111111", 0));
    }
